...
fun QrBarcodeScreen(onBackClick: () -> Unit, viewModel: QrBarcodeViewModel = hiltViewModel()) {
    ...
    QrBarcodeContent(
        ...
        actions = QrBarcodeActions(
            dismissGalleryResults = viewModel::dismissGalleryResults,
            selectGalleryResult = viewModel::selectGalleryResult,
            clearHistory = viewModel::clearHistory,
            delete = viewModel::delete,
            message = viewModel::message,
            scan = viewModel::scan,
            cancelImageScan = viewModel::cancelImageScan,
            retrySave = viewModel::retrySave,
            dismissResult = viewModel::dismissResult,
...
private data class QrBarcodeActions(
    val dismissGalleryResults: () -> Unit,
    val selectGalleryResult: (com.example.gscan.feature.tools.domain.BarcodeResult) -> Unit,
    val clearHistory: () -> Unit,
    val delete: (String) -> Unit,
    val message: (String) -> Unit,
    val scan: () -> Unit,
    val cancelImageScan: () -> Unit,
    val retrySave: () -> Unit,
    val dismissResult: () -> Unit,
    val reload: () -> Unit,
    val select: (com.example.gscan.feature.tools.domain.BarcodeHistoryItem) -> Unit,
)

@Composable
private fun QrBarcodeContent(
    state: QrBarcodeState,
    onBackClick: () -> Unit,
    onPickImage: () -> Unit,
    actions: QrBarcodeActions,
) {
    ...
    if (state.galleryCandidates.isNotEmpty()) AlertDialog(
        onDismissRequest = actions.dismissGalleryResults,
        ...
        text = {
            LazyColumn(
                ...
                items(state.galleryCandidates) { result ->
                    Card(
                        onClick = { actions.selectGalleryResult(result) },
        ...
        confirmButton = {},
        dismissButton = {
            TextButton(onClick = actions.dismissGalleryResults) { Text("Hủy") }
        },
    )
    ...
    if (deleteId != null || clear) AlertDialog(
        ...
        confirmButton = { TextButton(onClick = {
            if (clear) actions.clearHistory() else deleteId?.let(actions.delete)
    ...
    openUrl?.let { url -> AlertDialog(
        ...
        text = { SensitiveSelectionContainer(onCopyError = { actions.message("Không thể sao chép.") }) {
        ...
        confirmButton = { TextButton(onClick = {
            ...
            runCatching { context.startActivity(Intent(Intent.ACTION_VIEW, url.toUri()).addCategory(Intent.CATEGORY_BROWSABLE)) }
                .onFailure { actions.message("Không có ứng dụng phù hợp để mở đường dẫn.") }
        }) { Text("Mở") } },
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            item {
                ...
                Button(onClick = actions.scan, enabled = !state.busy && !state.unsaved, modifier = Modifier.fillMaxWidth()) {
                    Text(if (state.scanning) "Đang mở máy quét…" else "Quét bằng camera")
                }
                OutlinedButton(
                    onClick = {
                        if (state.readingImage) actions.cancelImageScan()
                        else onPickImage()
                    },
            ...
            state.selected?.let { selected -> item {
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        SensitiveSelectionContainer(onCopyError = { actions.message("Không thể sao chép.") }) {
                        ...
                        Row {
                            TextButton(onClick = {
                                ...
                                }.onSuccess { actions.message("Đã sao chép.") }.onFailure { actions.message("Không thể sao chép.") }
                            }) { Text("Sao chép") }
                            TextButton(onClick = {
                                ...
                                }, "Chia sẻ kết quả")) }.onFailure { actions.message("Không thể mở ứng dụng chia sẻ.") }
                            }) { Text("Chia sẻ") }
                        }
                        selected.result.webUrl()?.let { url -> TextButton(onClick = { openUrl = url }) { Text("Mở đường dẫn…") } }
                        if (state.unsaved) {
                            Text("Chưa lưu vào lịch sử.")
                            TextButton(onClick = actions.retrySave, enabled = !state.busy) { Text("Thử lưu lại") }
                        } else TextButton(onClick = actions.dismissResult, enabled = !state.busy) { Text("Đóng kết quả") }
                    }
                }
            } }
            item {
                ...
                if (state.historyError) {
                    ...
                    TextButton(onClick = actions.reload) { Text("Thử lại") }
                } else if (!state.loading && state.history.isEmpty()) Text("Chưa có kết quả quét.")
            }
            items(state.history, key = { it.id }) { item ->
                Card(onClick = { actions.select(item) }, enabled = !state.busy && !state.unsaved) {
...
@Composable
private fun QrBarcodeScreenPreview() {
    com.example.gscan.core.designsystem.theme.GScanTheme(darkTheme = false) {
        QrBarcodeContent(
            state = QrBarcodeState(loading = false),
            onBackClick = {},
            onPickImage = {},
            actions = QrBarcodeActions({}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}),
        )
