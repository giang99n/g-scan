...
- Room schema v6 lưu `Document + Page + OCR result/index + mẫu chữ ký + lịch sử barcode + thời điểm xóa mềm`; ingest dùng transaction, cancellation/lỗi ghi database được đối chiếu trước khi cleanup và startup reconciliation dọn orphan an toàn.
...
- Library đưa tài liệu vào thùng rác sau bước xác nhận; tài liệu đã xóa mềm không xuất hiện trong thư viện, tìm kiếm, OCR hay các bộ chọn tài liệu. Thùng rác cho phép khôi phục, xóa vĩnh viễn từng tài liệu và dọn toàn bộ sau xác nhận; metadata được xóa trước và file orphan được cleanup ngay hoặc qua startup reconciliation nếu thao tác file bị gián đoạn. Chưa có tự động xóa theo thời hạn.
- Library cho phép nhân bản tài liệu có trang thành một document mới mang hậu tố “(bản sao)”. Editor cho phép nhân bản một trang ngay sau trang gốc, tối đa 100 trang. Mỗi bản sao sở hữu file riêng, giữ rotation/chữ ký nhưng không sao chép OCR cũ; copy hoặc transaction lỗi được cleanup an toàn.
...
- OCR tiếng Việt/Anh bằng ML Kit Text Recognition Latin bundled model chạy on-device; xử lý nền theo document, lưu trạng thái/kết quả từng trang, hỗ trợ dừng/chạy lại, xem/chọn/sao chép văn bản và giữ kết quả cũ nếu lần chạy lại thất bại. Có thể xuất TXT UTF-8 theo thứ tự trang, chỉ lấy trang OCR thành công hoặc giữ placeholder cho đủ vị trí trang, rồi Save As/share bằng Android framework.
...
- Chưa có gộp trực tiếp nhiều PDF bên ngoài mà giữ text/vector, replace page, crop/filter/annotation, searchable PDF và các nhóm công cụ còn lại trong feature map.
