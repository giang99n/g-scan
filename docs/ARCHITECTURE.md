...
- Không cho xóa trang duy nhất. Xóa cả document từ Library là xóa mềm: Room đặt `deletedAtEpochMillis`, không sửa page/file nên có thể khôi phục nguyên thứ tự, góc xoay, OCR và chữ ký. Vì chỉ cập nhật Room, xóa mềm không chờ operation mutex đang giữ bởi OCR/export dài; các transaction page/OCR tự kiểm tra document còn active trước khi commit. Mọi query thư viện/search/detail chỉ đọc document chưa xóa. Thùng rác cho phép restore, xóa vĩnh viễn từng document hoặc dọn toàn bộ sau xác nhận; khi xóa vĩnh viễn Room xóa metadata/pages bằng foreign-key cascade trước, rồi file được cleanup trong `NonCancellable`. Nếu cleanup lỗi, startup reconciliation sẽ dọn thư mục orphan ở lần mở app tiếp theo. Mọi mutation xóa/restore kiểm tra số dòng tác động để không báo thành công giả khi trạng thái đã thay đổi.
...
- Nhân bản document tái sử dụng `ComposeDocumentUseCase`/pipeline compose: lấy snapshot page ID, copy tuần tự sang thư mục document mới rồi transaction insert metadata. Tên mới có hậu tố “(bản sao)”; page giữ rotation/chữ ký, có ID/source file riêng và không sao chép OCR.
- Nhân bản page dùng `ManageDocumentPagesUseCase → DocumentRepository.duplicatePage`: source được copy sang tên file chưa dùng trong cùng thư mục, sau đó transaction chèn page mới ngay sau page gốc và rewrite position bằng khoảng tạm để giữ unique index. Page mới giữ rotation/chữ ký nhưng không có OCR result. Nếu commit không xảy ra thì chỉ file mới được cleanup; nếu cancellation đến sau commit, repository xác minh page ID để không xóa nhầm file đã được Room tham chiếu. Replace page chưa được triển khai.
- Chữ ký đi qua `SignatureViewModel → SignatureUseCases → SignatureRepository`. Room v4 thêm `signature_templates(id, name, ink)` và `pages.signatureInk` mặc định `[]` bằng migration 3→4. Nét ký là JSON danh sách điểm chuẩn hóa 0–1, giới hạn 100 nét/8000 điểm; tọa độ trang tính sau EXIF nhưng trước rotation do người dùng. Editor chuyển tọa độ ngược góc xoay khi lưu; preview/thumbnail/PDF dùng chung `SignatureInk` để xoay và vẽ lại. Lưu chữ ký + updatedAt document trong transaction dưới operation mutex; xóa mẫu không ảnh hưởng nét đã đặt. Page copy trong gộp/trích xuất giữ metadata chữ ký. Export ghép nét lên bitmap mutable trước JPEG, không sửa source; OCR tiếp tục đọc source không có lớp chữ ký. Bản nháp giữ qua ViewModel khi xoay màn hình nhưng chưa phục hồi sau process death.
- Gộp/trích xuất dùng `ComposeDocumentUseCase` → `DocumentRepository.composeDocument`: UI chọn danh sách document theo thứ tự và trang/khoảng trang; ViewModel chuyển lựa chọn thành page ID. Repository giữ operation mutex, xác minh các page ID còn tồn tại, copy tuần tự source sang thư mục tài liệu mới rồi insert Document + Pages bằng transaction. Góc xoay/EXIF được giữ; mỗi trang có ID và file riêng, không chia sẻ source với bản gốc. Callback copy báo tiến trình. Khi lỗi/hủy, chỉ cleanup thư mục mới nếu Room xác nhận chưa commit; nếu đã commit trả ID thành công, nếu không đọc được Room để reconciliation xử lý sau. Không đổi schema/dependency; OCR mới chạy độc lập khi người dùng yêu cầu.
...
- Export TXT đọc snapshot Document/Page/OCR dưới operation lock, ghi UTF-8 vào `cache/text_exports` qua file `.part` rồi rename khi hoàn tất. Hai chế độ cho phép chỉ xuất trang OCR thành công hoặc giữ placeholder theo toàn bộ vị trí trang; kết quả cũ được đánh dấu nếu lần OCR mới thất bại/chưa hoàn tất. UI vô hiệu hóa và hủy tham chiếu export cũ khi OCR thay đổi. Save As dùng SAF; share chỉ cấp content URI của `text_exports/` qua `FileProvider`, không expose source page.
...
Schema hiện tại là version 7. Bảng `documents` có `deletedAtEpochMillis` nullable để phân biệt tài liệu hoạt động và trong thùng rác. Bảng `pages` có foreign key cascade tới `documents` và unique index `(documentId, position)`. `ocr_results` cascade theo `pageId`; FTS index được xóa tường minh khi xóa page/document. Migration `1 → 2` giữ các document demo cũ nhưng đưa chúng về `DRAFT`, `pageCount = 0` vì version 1 chưa có source page thật; migration `2 → 3` thêm OCR result và FTS; migration `3 → 4` thêm mẫu và metadata chữ ký; migration `4 → 5` thêm `barcode_history`; migration `5 → 6` thêm nullable `deletedAtEpochMillis`; migration `6 → 7` thêm favorite, folder và tag mà không thay đổi source page.

### QR/barcode

- `QrBarcodeViewModel → BarcodeScanner → GoogleBarcodeScanner` mở camera UI của Google Play services; adapter chỉ trả model domain gồm raw text/format/type, không đưa Barcode SDK hoặc Context vào domain. Application context không giữ Activity qua xoay màn hình. ViewModel chặn mở nhiều phiên quét; hủy/lỗi có trạng thái riêng.
- `QrBarcodeViewModel → GalleryBarcodeScanner → MlKitGalleryBarcodeScanner` nhận content URI tạm từ Photo Picker, decode có giới hạn cạnh 2048 px ngoài main thread và chạy model bundled on-device. URI/ảnh không được copy hay lưu. Một mã đi thẳng vào luồng kết quả hiện có; nhiều mã được khử trùng lặp và yêu cầu người dùng chọn trước khi lưu.
- Kết quả hiển thị trước khi lưu bằng `BarcodeHistoryUseCases → BarcodeHistoryRepository → BarcodeHistoryDao`. UUID cố định cho một kết quả và INSERT IGNORE cho phép retry an toàn. Lỗi ghi không xóa kết quả; người dùng vẫn sao chép/chia sẻ hoặc thử lưu lại. Kết quả chưa commit chỉ giữ trong ViewModel, chưa khôi phục sau process death.
- `barcode_history(id, content, format, type, scannedAt)` độc lập Document/Page; đọc qua Flow, xóa từng mục hoặc toàn bộ sau xác nhận. Không log payload; clipboard gắn cờ sensitive. Lịch sử có thể chứa thông tin Wi-Fi/liên hệ, được thông báo ngay trong màn hình.
- Chỉ URI tuyệt đối HTTP/HTTPS có host và không có user-info mới có nút mở; vẫn cần người dùng xác nhận URL đầy đủ. Không tự mở link, kết nối Wi-Fi hoặc nhập contact từ payload.
- Google Code Scanner 16.1.0 được Google công bố miễn phí thuộc ML Kit; không yêu cầu billing/API key và không thêm CAMERA permission. Scanner module tải qua Play services (`barcode_ui`), lần đầu có thể cần mạng; không cam kết hoạt động trên thiết bị thiếu GMS. Xem https://developers.google.com/ml-kit/vision/barcode-scanning/code-scanner và https://developers.google.com/ml-kit/guides. SDK chịu Google APIs/ML Kit terms và có thể gửi telemetry theo chính sách ML Kit; không tích hợp dịch vụ cloud tính phí.
- ML Kit Barcode Scanning bundled 17.3.0 không yêu cầu API key/billing, tăng APK khoảng 2,4 MB và sẵn sàng offline ngay sau cài đặt. Photo Picker không yêu cầu quyền đọc toàn bộ thư viện. Xem https://developers.google.com/ml-kit/vision/barcode-scanning/android.
