...
Schema hiện tại là version 6. Bảng `documents` có `deletedAtEpochMillis` nullable để phân biệt tài liệu hoạt động và trong thùng rác. Bảng `pages` có foreign key cascade tới `documents` và unique index `(documentId, position)`. `ocr_results` cascade theo `pageId`; FTS index được xóa tường minh khi xóa page/document. Migration `1 → 2` giữ các document demo cũ nhưng đưa chúng về `DRAFT`, `pageCount = 0` vì version 1 chưa có source page thật; migration `2 → 3` thêm OCR result và FTS; migration `3 → 4` thêm mẫu và metadata chữ ký; migration `4 → 5` thêm `barcode_history`; migration `5 → 6` thêm nullable `deletedAtEpochMillis` nên toàn bộ tài liệu cũ tiếp tục hoạt động.
